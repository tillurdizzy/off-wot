...
$query = "DELETE FROM eventdata WHERE PRIMARY_ID='".$_eventID."'";
$qry_res = mysqli_query($con,$query);
if ($qry_res) {
	$arr = array('msg' => "Delete successful", 'result' => $qry_res, 'params' => $_eventID);
	$jsn = json_encode($arr);
	echo($jsn);
} else {
	$arr = array('msg' => "Error deleting record", 'result' => $qry_res,'params' => $_eventID);
	$jsn = json_encode($arr);
	echo($jsn);
}
?>