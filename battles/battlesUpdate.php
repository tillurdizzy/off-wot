...
if ($qry_res) {
	$arr = array('msg' => "Update successful", 'result' => mysqli_insert_id($con));
	$jsn = json_encode($arr);
	echo($jsn);
} else {
	$arr = array('msg' => "Error updating record", 'result' => $qry_res,'params' => $qry_res);
	$jsn = json_encode($arr);
	echo($jsn);
}
?>