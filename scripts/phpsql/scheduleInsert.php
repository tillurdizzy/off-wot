...
$name = mysql_real_escape_string($data->member);
...
$query = "INSERT INTO schedule(beg,end,inputBeg,inputEnd,name,tier_high,tier_low,zone)
VALUES(
'" . $beg . "', " .
"'" . $end . "', " .
"'" . $inputBeg . "', " .
...
if ($qry_res) {
	$arr = array('msg' => "Insert successful", 'result' => $qry_res, 'params' => $beg . ":" + $end . ":" . $name . ":" . $zone);
	$jsn = json_encode($arr);
	echo($jsn);
} else {
	$arr = array('msg' => "Error inserting record", 'result' => $qry_res,'params' => $beg . ":" + $end . ":" . $name . ":" . $zone);
	$jsn = json_encode($arr);
	echo($jsn);
}
?>